function [ G_0_4D, G_0_2D ] = G_0_split_function( r, omega, delta_V, epsilon_ref, N, wave_type )
...
for j = 1:N-1                            % Loop through all discretized volume locations
    for i = j+1:N                        % Loop through all discretized volume locations (again)
        ...
        G_0_NF = ( exp(1i*k*r_ij_mag)/(4*pi*r_ij_mag) ) * (1./((k*r_ij_mag)^2)) * ( -(1.*I) + (3.*r_ij_outer_r_ij));
        G_0_IF = ( exp(1i*k*r_ij_mag)/(4*pi*r_ij_mag) ) * (1.i/(k*r_ij_mag))*( (1.*I) - (3.*r_ij_outer_r_ij) );
        G_0_FF = ( exp(1i*k*r_ij_mag)/(4*pi*r_ij_mag) ) * ( (1.*I) - (1.*r_ij_outer_r_ij)  );
               
        
        % Total contribution accounts for all wave types 
        if wave_type == "total"
            
            G_0_split = G_0_FF+G_0_IF +G_0_NF;
        ...
        if wave_type == "evan"
            if j<=N/2 && i>N/2 || i<=N/2 && j>N/2
                ...
                G_0_split = G_0_NF;%0.6 or + 0.5*G_0_IF
