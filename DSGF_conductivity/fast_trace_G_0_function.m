%function [ G_0_2D ] = G_0_function( r, omega, delta_V, epsilon_ref, N, wave_type )
function [ G_0_4D, G_0_2D ] = fast_trace_G_0_function( r, omega, delta_V, epsilon_ref, N, wave_type )
...
for j = 1:N-1                            % Loop through all discretized volume locations
    ...
    % parfor i = j+1:N                        % Loop through all discretized volume locations (again)
...
G_0_2D = reshape(permute(G_0_4D,[3,1,4,2]),[3*N,3*N]);
G_0_2D = G_0_2D + G_0_2D.' + self_terms;  %% ORIGINAL



%{
% Masks
mask_ij = tril(true(N));   % lower triangle in (i,j)
mask_kl = tril(true(3));   % lower triangle in (k,l)

nt_ij = nnz(mask_ij);  % number of unique (i,j)
nt_kl = nnz(mask_kl);  % number of unique (k,l)

% Preallocate packed storage
A_packed = zeros(nt_ij, nt_kl);

% Loop over triangular indices of (k,l)
idx_kl = find(mask_kl);
for t = 1:nt_kl
    [k,l] = ind2sub([m,m], idx_kl(t));
    slice = G_0_4D(:,:,k,l);  % n×n block
    A_packed(:,t) = slice(mask_ij);
end
%}
