function [ G_sys_2D, Trans, Q_omega_bulk, Q_omega_subvol ] = fast_trace_direct_function( omega, r, epsilon, epsilon_ref, delta_V, T_vector, ind_bulk, wave_type )
...
[~, G_0_2D] = G_0_function(r, omega, delta_V, epsilon_ref, N, wave_type); %no changes so far
%[ G_0_2D] = fast_trace_G_0_function(r, omega, delta_V, epsilon_ref, N, wave_type); %no changes so far

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% Populate deterministic interaction matrix %
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

[ A_2D, ~ ] = A_matrix_function(G_0_2D, omega, epsilon, epsilon_ref, delta_V, N);  %no changes so far
%[ A_2D, ~ ] = fast_trace_A_matrix_function(G_0_2D, omega, epsilon, epsilon_ref, delta_V, N);  %no changes so far
...
t1 = toc;
%G_sys_2D = A_2D\G_0_2D;  % System Green's function matrix, 2D matrices


G_0_3N_1 = ones(3*N,1);
%G_0_3N_1 = ones(3*N/2,1);
i = 0;
for column = 1:3*N/2    
%for column = (3*N/2)+1:3*N           
