...
function [] = DSGF_main_conductivity_new(description, discretization, material, T, T_conductance, epsilon_ref, omega, wave_type, output, discretization_type, L_char, delta_V, d, N_cross_section, structure)
...
elseif strcmp('user_defined',discretization_type)
    ...
    elseif strcmp('coating',structure)
        ...
        Nmax_SiC_2 = 720;
        Nmin_SiC_3 = 721;
        Nmax_SiC_3 = 1080;% N_imaginary_plane
        Nmin_SiC_4 = 1081;
        Nmax_SiC_4 = 1440;
        Nmin_SiC_5 = 1441;
        Nmax_SiC_5 = 1800;
        ...
        r_conductivity = r(Nmin_SiC_3:Nmax_SiC_4,:);
        
        r_1_SiC = r(Nmin_SiC_1:Nmax_SiC_1,:);
        r_2_SiC = r(Nmin_SiC_2:Nmax_SiC_2,:);
        r_1_Au = r(Nmin_Au_1:Nmax_Au_1,:);
        r_2_Au = r(Nmin_Au_2:Nmax_Au_2,:);
        %{
        r_1 = [r_1_SiC,r_1_Au];
        r_2 = [r_2_SiC,r_1_Au];
        %}           
...
elseif strcmp('user_defined',discretization_type)
   ...
   % elseif strcmp('coating',structure)
   % discretization_plotting_user_defined_conductivity(r, L_sub_vector, N, show_axes, output, filePath_st.main, N1,r_conductivity);
   end 
end
...
switch (material)
	case Material.SiO2
	    ...
	    epsilon = SiO2_dielectric_function(omega, constants); % (N_omega x 1) vector of all dielectric functions for every frequency

	case Material.SiC

	    %epsilon = SiC_dielectric_function(omega, constants); % (N_omega x 1) vector of all dielectric functions for every frequency

 	    epsilon = SiC_poly_dielectric_function(omega); % (N_omega x 1) vector of all dielectric functions for every frequency
	...
	case Material.Si3N4
	    ...
	    epsilon = SiN_dielectric_function(omega, constants); % (N_omega x 1) vector of all dielectric functions for every frequency

    case Material.Au 

	    epsilon = Au_dielectric_function(omega); % (N_omega x 1) vector of all dielectric functions for every frequency

        
    case Material.SiC_Au %Added by Livia on 2025/07/29  
        
        epsilon_SiC = SiC_poly_dielectric_function(omega); % (N_omega x 1) vector of all dielectric functions for every frequency  
        epsilon_Au = Au_dielelectric_function(omega); %5+1i*0.1; %SiC_poly_dielectric_function(omega); % %Au_dielelectric_function(omega); % (N_omega x 1) vector of all dielectric functions for every frequency
...
G_omega_ij = zeros(N,N,N_omega);
%Trans = zeros(N,N,N_omega);
spectral_therm_conductivity = zeros(N_omega, length(T_conductance));
%spectral_therm_conductivity_xx = zeros(N_omega, length(T_conductance));

%{
num = ((h_bar.*omega).^2).*exp((h_bar.*omega)./(k_b.*T_conductance));
denom = k_b.*(T_conductance.^2).*((exp((h_bar.*omega)./(k_b.*T_conductance)) - 1).^2);
dtheta_dT = num./denom; %vector 1xN_temp_conductance
%}
    ...
    t1 = toc;
        ...
        %calculate thermal conductivity
            ...
            if strcmp('bare',structure)
                    ...
                    % Bare structure %
                    %%%%%%%%%%%%%%%%
                    %Calculates Trans matrix for a single material
                    ...
                    [spectral_therm_conductivity(omega_loop,:), spectral_conductance(omega_loop,:)] = spectral_therm_conductivity_conductance_function(Trans, T_conductance, r, L_sub_vector(1), omega(omega_loop),Nmin_SiC_1, Nmin_SiC_2, Nmax_SiC_1, Nmax_SiC_2, N_cross_section);
                    
                    %[ Trans ] = direct_function_trans(omega(omega_loop), r, epsilon(omega_loop)*ones(N,1), epsilon_ref, delta_V_vector, T_vector, ind_bulk, wave_type);
                    % In the fast_trace_direct_function_trans, only half of the terms are calculated. 
                    %[ Trans ] = fast_trace_direct_function_trans(omega(omega_loop), r, epsilon(omega_loop)*ones(N,1), epsilon_ref, delta_V_vector, T_vector, ind_bulk, wave_type, T_conductance);
                    %[spectral_therm_conductivity(omega_loop,:)] = spectral_therm_conductivity_function_coating(Trans, T_conductance, r, L_sub_vector(1), omega(omega_loop),Nmin_SiC_1, Nmin_SiC_2, Nmax_SiC_1, Nmax_SiC_2, N_cross_section);
                    
            elseif strcmp('coating',structure)
                    %%%%%%%%%%%%%%%%
                    % Heterostructure %
                    %%%%%%%%%%%%%%%%
                    %Case of different materials
                    %Assign the appropriate epsilon for each subvolume:
        
                    epsilon = ones(N,1);
                    for i = 1:N
                        if i <= Nmax_SiC_6 %Modify according to the discretization
                            epsilon(i) =  epsilon_SiC(omega_loop);
                        else
                            epsilon(i) =  epsilon_Au(omega_loop); %epsilon_Au; %; %for a frequency-independent material
                        end     
                    end 
                   [ Trans ] = direct_function_trans(omega(omega_loop), r, epsilon(omega_loop)*ones(N,1), epsilon_ref, delta_V_vector, T_vector, ind_bulk, wave_type);
                   [spectral_therm_conductivity(omega_loop,:), spectral_conductance(omega_loop,:) ] = spectral_therm_conductivity_conductance_function(Trans, T_conductance, r, L_sub_vector(1), omega(omega_loop),Nmin_SiC_3, Nmin_SiC_4, Nmax_SiC_3, Nmax_SiC_4, N_cross_section);
                    
                    %[ Trans ] = direct_function(omega(omega_loop), r, epsilon(omega_loop)*ones(N,1), epsilon_ref, delta_V_vector, T_vector, ind_bulk, wave_type);
                    
                    %[ Trans ] = direct_function(omega(omega_loop), r, epsilon(:), epsilon_ref, delta_V_vector, T_vector, ind_bulk, wave_type, T_conductance);
                    %with coated ends
                    %Nmin_SiC_1, Nmin_SiC_2, Nmax_SiC_1, Nmax_SiC_2 
                    % [spectral_therm_conductivity(omega_loop,:) ] = spectral_therm_conductivity_function_coating(Trans, T_conductance, r, L_sub_vector(1), omega(omega_loop),Nmin_SiC_3, Nmin_SiC_4, Nmax_SiC_3, Nmax_SiC_4, N_cross_section);
                    % [spectral_therm_conductivity(omega_loop,:)] = spectral_therm_conductivity_function_coating(Trans, T_conductance, r, L_sub_vector(1), omega(omega_loop),Nmin_SiC_1, Nmin_SiC_2, Nmax_SiC_1, Nmax_SiC_2, N_cross_section);
...
%}    
        ...
        save_TRANS_matrix(output, filePath_st, omega_loop, Trans);
    ...
    disp([num2str(omega(omega_loop)) ':' num2str(spectral_therm_conductivity(omega_loop,:)) ])
...
%}
figure(2)
    plot(omega, spectral_therm_conductivity(:,1), 'linewidth', 3)
    legend('x-direction', 'y-direction', 'z-direction', 'location', 'best')
    title('Spectral thermal conductivity')
    xlabel('Frequency, \omega (rad/s)')
    ylabel('Spectral thermal conductivity')
    set(gca, 'fontsize', 20)
    
figure(3)
    plot(omega, spectral_conductance(:,1), 'linewidth', 3)
    legend('x-direction', 'y-direction', 'z-direction', 'location', 'best')
    title('Spectral conductance')
    xlabel('Frequency, \omega (rad/s)')
    ylabel('Spectral conductance')
    set(gca, 'fontsize', 20)    

if omega(1) > omega(2)
    total_therm_conductivity_x = (1/(2*pi)).*trapz(flipud(omega), flipud(spectral_therm_conductivity));
    total_conductance = (1/(2*pi)).*trapz(flipud(omega), flipud(spectral_conductance));
    else
    total_therm_conductivity_x = (1/(2*pi)).*trapz(omega, spectral_therm_conductivity);
    total_conductance = (1/(2*pi)).*trapz(omega, spectral_conductance);
end
%{
if omega(1) > omega(2)
    total_therm_conductivity_Lindsay = trapz(flipud(omega), flipud(spectral_therm_conductivity_Lindsay));
    else
    total_therm_conductivity_Lindsay = trapz(omega, spectral_therm_conductivity_Lindsay);
end
%}
%%%%%%%%%%%%%%%%
% Save results %
